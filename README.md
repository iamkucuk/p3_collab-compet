...
The observation space consists of 8 variables corresponding to the position and velocity of the ball and racket, for 3 frames, resulting a total state size of 24. (Example state can be observed from Report Notebook) Each agent receives its own, local observation.  Two continuous actions are available, corresponding to movement toward (or away from) the net, and jumping. 
...
Follow the instructions in `Report.ipynb` and run all of the cells. Detailed explainations of classes and functions are in the py files.
